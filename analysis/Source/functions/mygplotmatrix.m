...
% clr = scatterinternal.stats.cycleLineProperties(ng,clr);
      ...
      if ((i==j) && XvsX)
          if (dohist||doghistBar||doghistStair)
              ...
              if dohist
                  ...
                  if ng == 1
                      set(hhdiag(i),'FaceColor',clr{2});
                  end
              elseif doghistBar
                  hhdiag(i,:) = internal.stats.plotGroupedHist(reshape(y(:,i,:),[m k]),g,'Color',clr{1},'DisplayStyle','bar','Norm','pdf');
              elseif doghistStair
                  hhdiag(i,:) = internal.stats.plotGroupedHist(reshape(y(:,i,:),[m k]),g,'Color',clr{1},'DisplayStyle','stairs','Norm','pdf');
              end
