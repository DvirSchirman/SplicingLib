function  h=myviolinplot_mean( data, color_map, d_min, d_max )
...
%     figure
    hold on
%     h=[];
    data(cellfun(@isempty,data))={randn(10,1)+(d_min-1e4)};
%     data(cellfun(@(x) nansum(x==x(1))==sum(~isnan(x)),data))={[data{cellfun(@(x) nansum(x==x(1))==sum(~isnan(x)),data)}; randn(2,1)+(d_min-1e4)]};
    ...
    for i=1:M
        ...
        v=violinplot_mean([randn(length(data{i}),i-1)+(d_min-1e4),data{i},randn(length(data{i}),M-i)+(d_min-1e4)]);
