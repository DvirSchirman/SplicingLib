classdef Violin_mean < handle
    ...
    methods
        function obj = Violin_mean(data, pos, varargin)
            ...
            if bw<0.16
                [density, value] = ksdensity(data, 'bandwidth', bw*3);
            end
            if bw < 0.016
                [density, value] = ksdensity(data, 'bandwidth', bw*30);
            end
...
%                 fill([pos-0.015 pos+0.015 pos+0.015 pos-0.015], ...
...
%             obj.MedianPlot = plot([pos-0.1 pos+0.1], [quartiles(2) quartiles(2)],'color',[0,0,0],'linewidth',4);
            obj.MedianPlot = plot([pos-0.1 pos+0.1], [nanmean(data) nanmean(data)],'color',[0,0,0],'linewidth',4);
