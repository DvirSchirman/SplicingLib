...
[H,Ax,bigAx] = mygplotmatrix(X,[],[],{[0,0,0]+grey_alpha;color_orange},'o',5,[],'hist',varNames,varNames);

% Format plot:

set(H(logical(eye(numVars))),'EdgeColor',[0,0,0]+grey_alpha)
sXlabels = get(Ax,'XLabel');
sYlabels = get(Ax,'YLabel');
% set([sXlabels{:}],'FontWeight','bold','Color',[0 0 0])
% set([sYlabels{:}],'FontWeight','bold','Color',[0 0 0])
set(get(bigAx,'Title'),'String','{\bf Correlation Matrix}')
if isempty(AXlim)
    for i=1:numVars
        AXlim(i,:)=Mu(i)+(1.1)*ZLims*Sigma(i);
    end
elseif ~isempty(find(isnan(AXlim(:))))
    for i=1:numVars
        if isempty(find(isnan(AXlim(i,:))))
            continue;
        else
            AXlim(i,:)=Mu(i)+(1.1)*ZLims*Sigma(i);
        end
...
for i = 1:numVars
    ...
    for j = 1:numVars
        ...
        if i==5
            Ax(i,j).YAxis.Exponent=-3;
        end
        if j==5
            Ax(i,j).XAxis.Exponent=-3;
        end
