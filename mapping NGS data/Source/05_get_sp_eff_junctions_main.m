...
outPath='../Data/Out_sp_eff_junctions/';
...
for i=1:length(samplesDirs)
	...
	for j=1:length(fileNames)
		cmd = sprintf('bsub -q new-short -R "rusage[mem=4096]" matlab -nojvm -nodesktop -nosplash -r ''get_sp_eff_junctions %s %s %s''',currSamplePath, fileNames{j}, currOutPath);
		system(cmd);
	end
end
