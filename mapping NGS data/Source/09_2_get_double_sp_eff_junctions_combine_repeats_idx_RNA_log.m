function get_double_sp_eff_junctions_combine_repeats_idx_RNA_log_log(idx)
...
files = dir([in_sample_folder '*_RNA_log_*']);
files={files.name};
...
for i=1:length(files)
    db(i)=load(sprintf('%s%s/Sp_RNA_log_%d.idx%d.mat',in_sample_folder,files{i},inds(i),idx));
end
...
% double_sp_eff_tbl.other_abundance=sum(other_abundance_mat,2);
...
save(sprintf('%sRNA_log_junctions_idx%d.mat',out_folder,idx),'double_sp_eff_tbl');
